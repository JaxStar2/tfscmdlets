...
namespace TfsCmdlets.PSWindows.TfsApi.Services
{
    ...
    public class WorkItemQueryService : IWorkItemQueryService
    {
        ...
        {
            return new WorkItemQueryDefinitionAdapter(GetItem<QueryDefinition2>(item, scope, project, collection, server, credential));
        }
        ...
        public IEnumerable<IWorkItemQueryDefinitionAdapter> GetDefinitions(object item, WorkItemQueryScope scope, object project, object collection, object server,
            object credential)
        {
            foreach (var qd in GetItems<QueryDefinition2>(item, scope, project, collection, server, credential))
            {
                yield return new WorkItemQueryDefinitionAdapter(qd);
            }
        }
        ...
        public IWorkItemQueryFolderAdapter GetFolder(object item, WorkItemQueryScope scope, object project, object collection,
            object server, object credential)
        {
            return new WorkItemQueryFolderAdapter(GetItem<QueryFolder2>(item, scope, project, collection, server, credential));
        }
        ...
        public IEnumerable<IWorkItemQueryFolderAdapter> GetFolders(object item, WorkItemQueryScope scope, object project, object collection, object server,
            object credential)
        {
            foreach (var qf in GetItems<QueryFolder2>(item, scope, project, collection, server, credential))
            {
                yield return new WorkItemQueryFolderAdapter(qf);
            }
        }

        private T GetItem<T>(object item, WorkItemQueryScope scope, object project, object collection,
            object server, object credential)
            where T : QueryItem2
        {
            ...
            where T : QueryItem2
        {
            ...
            while (true)
            {
                switch (query)
                {
                    ...
                    case Guid g:
                        {
                            query = new Predicate<QueryItem2>(qi => qi.Id == g);
                            continue;
                        }
                    case string s when !string.IsNullOrEmpty(s):
                        {
                            var normalizedPath = NormalizePath(s, rootPath);
                            query = new Predicate<QueryItem2>(qi => qi.Path.IsLike(normalizedPath));  //  || qi.Name.IsLike(s)
                            continue;
                        }
